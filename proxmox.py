...
#
# Updated 2025 by Ouroborosng:
# - Improved Naming for Interface Filters: Introduced `include_iface_patterns`
#   and `exclude_iface_patterns` for better clarity and configuration flexibility.
# - Secure Authentication: Removed password-based login, now exclusively supports
#   authentication via Proxmox API token for enhanced security.
...
class ProxmoxVMList(list):
    ...
    def get_names(self):
        if self.ver >= 4.0:
            return [vm['name'] for vm in self if not vm.get('template')]
        else:
...
class ProxmoxPool(dict):
    def get_members_name(self):
        return [member['name'] for member in self['members'] if (member['type'] == 'qemu' or member['type'] == 'lxc') and not member.get('template')]
...
class ProxmoxAPI(object):
    def __init__(self, options, config_path):
        ...
        if not options.url or not options.username or not options.token or not options.secret:
                ...
                with open(config_path, "r") as config_file:
                    ...
                    if not options.secret:
                        try:
                            ...
                            options.secret = None
                    if not options.include_iface_patterns:
                        options.include_iface_patterns = config_data["include_iface_patterns"]
                    if not options.exclude_iface_patterns:
                        options.exclude_iface_patterns = config_data["exclude_iface_patterns"]
        ...
        elif not options.token or not options.secret:
            raise Exception(
                'Missing mandatory parameter --token and --secret (or PROXMOX_TOKEN and PROXMOX_SECRET or "token" and "secret" key in config file).')
...
### GET HOSTNAME for LXC-Containers
    ...
    def valid_network_interface(self, network):
        ...
        # Network interface include/exclude patterns are mutually exclusive
        if len(self.options.include_iface_patterns) > 0:
            for regex in self.options.include_iface_patterns:
                if re.match(regex, network["name"]):
                    return True
            return False
        
        if len(self.options.exclude_iface_patterns) > 0:
            for regex in self.options.exclude_iface_patterns:
                if re.match(regex, network["name"]):
                    return False
            return True
...
def main_list(options, config_path):
    ...
    for node in proxmox_api.nodes().get_names():
        ...
        for vm in node_hostvars:
            # Exclude VM template
            if results['_meta']['hostvars'][vm].get('proxmox_template') == 1:
                continue
...
def main():
    ...
    parser.add_option('--include-iface', default=os.environ.get("INCLUDE_IFACE_PATTERNS", []), action="append", dest='include_iface_patterns', help="Only include network interfaces whose names match these regex patterns.")
    parser.add_option('--exclude-iface', default=os.environ.get("EXCLUDE_IFACE_PATTERNS", []), action="append", dest='exclude_iface_patterns', help="Exclude network interfaces whose names match these regex patterns.")
